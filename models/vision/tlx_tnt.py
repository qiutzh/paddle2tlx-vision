...
from tensorlayerx.nn.initializers import TruncatedNormal, Constant

# from ..base.theseus_layer import Identity
# from ....utils.save_load import load_dygraph_pretrain, load_dygraph_pretrain_from_url
from utils.load_model_tlx import _load_pretrained
# from utils.load_model_pd import _load_pretrained
...
trunc_normal_ = TruncatedNormal(stddev=.02)
zeros_ = Constant(value=0.)
ones_ = Constant(value=1.)


# class Identity(nn.Layer):  # add
class Identity(nn.Module):  # add
    def __init__(self):
...
def drop_path(x, drop_prob=0., training=False):
    ...
    keep_prob = tlx.ops.convert_to_tensor(1 - drop_prob)
...
class Attention(nn.Module):
    def __init__(self,
        ...
        self.qk = nn.Linear(in_features=dim, out_features=hidden_dim * 2, b_init=qkv_bias)
        # self.v = nn.Linear(dim, dim, bias_attr=qkv_bias)
        self.v = nn.Linear(in_features=dim, out_features=dim, b_init=qkv_bias)
    ...
    def forward(self, x):
        ...
        attn = tlx.ops.matmul(q, k.transpose((0, 1, 3, 2))) * self.scale
        # attn = nn.functional.softmax(attn, axis=-1)
        attn = tlx.ops.softmax(attn, axis=-1)
        # attn = self.attn_drop(attn)

        # x = paddle.matmul(attn, v)
        x = tlx.ops.matmul(attn, v)
...
class PixelEmbed(nn.Module):
    def __init__(self,
        ...
        self.proj = nn.Conv2d(
            in_channels=in_chans, out_channels=self.in_dim, kernel_size=7, padding=3, stride=stride, data_format='channels_first')
...
class TNT(nn.Module):
    def __init__(self,
        ...
        self.proj = nn.Linear(in_features=num_pixel * in_dim, out_features=embed_dim)
        self.norm2_proj = norm_layer(embed_dim)

        self.cls_token = self.create_parameter(
            shape=(1, 1, embed_dim), default_initializer=zeros_)  # TODO - paddle 方式初始化
    ...
    def _init_weights(self, m):
        if isinstance(m, nn.Linear):
            ...
            # zeros_(m.bias)  # TODO - AttributeError: 'LayerNorm' object has no attribute 'bias'
            # ones_(m.weight)
            # zeros_(m.beta_init._value)
            # ones_(m.gamma_init._value)
            pass
