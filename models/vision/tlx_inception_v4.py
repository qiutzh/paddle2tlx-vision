...
from tensorlayerx.nn import Conv2d, GroupConv2d, BatchNorm, Linear, Dropout, AdaptiveAvgPool2d, MaxPool2d, AvgPool2d
...
class InceptionStem(nn.Module):
    ...
    def forward(self, inputs):
        conv = self._conv_1(inputs)  # TODO result is diff
        ...
        concat = tlx.ops.concat([pool1, conv2], axis=1)
...
class InceptionA(nn.Module):
    def __init__(self, name):
        ...
        self._pool = AvgPool2d(kernel_size=3, stride=1, padding=1, data_format='channels_first')
    ...
    def forward(self, inputs):
        ...
        concat = tlx.ops.concat([conv1, conv2, conv3, conv4], axis=1)
        return concat
...
class ReductionA(nn.Module):
    ...
    def forward(self, inputs):
        ...
        concat = tlx.ops.concat([pool1, conv2, conv3], axis=1)
        return concat
...
class InceptionV4DY(nn.Module):
    def __init__(self, num_classes=1000):
        ...
        self.out = Linear(
            ...
            W_init=tlx.nn.initializers.random_uniform(-stdv, stdv),
            b_init=tlx.initializers.xavier_uniform())  # TODO - paddle is not set False

    def forward(self, inputs):
        x = self._inception_stem(inputs)  # TODO - result is diff
        ...
        x = tlx.ops.squeeze(x, axis=[2, 3])  # same
        # x = self._drop(x)  # TODO - not same in here, comment this line, result is same
        x = self.out(x)
        return x
...
def InceptionV4(pretrained=False, **kwargs):
    ...
    _load_pretrained(pretrained, model, MODEL_URLS["InceptionV4"])
    return model
