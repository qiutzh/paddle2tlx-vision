...
class BottleneckBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        y = paddle.add(x=short, y=conv2)  # TODO
        # y = F.relu(y)
        y = tlx.ops.relu(y)
        return y
...
class SELayer(nn.Module):
    def __init__(self, num_channels, num_filters, reduction_ratio, name=None):
        ...
        self.pool2d_gap = AdaptiveAvgPool2d(output_size=1, data_format='channels_first')
        ...
        self.squeeze = Linear(
            ...
            b_init=tlx.nn.initializers.xavier_uniform())
    ...
    def forward(self, input):
        ...
        pool = tlx.ops.reshape(pool, shape=[-1, self._num_channels])
        squeeze = self.squeeze(pool)
        # squeeze = F.relu(squeeze)
        squeeze = tlx.ops.relu(squeeze)
        excitation = self.excitation(squeeze)
        # excitation = F.sigmoid(excitation)
        excitation = tlx.ops.sigmoid(excitation)
        # excitation = paddle.reshape(
        #     excitation, shape=[-1, self._num_channels, 1, 1])
        excitation = tlx.ops.reshape(
            excitation, shape=[-1, self._num_channels, 1, 1])
...
class RegNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        y = tlx.ops.reshape(y, shape=[-1, self.pool2d_avg_channels])
        y = self.out(y)
        return y


def RegNetX_200MF(pretrained=False, **kwargs):
    model = RegNet(
        w_a=36.44,
        w_0=24,
        w_m=2.49,
        ...
        **kwargs)
    _load_pretrained(
        pretrained, model, MODEL_URLS["RegNetX_200MF"])
    return model
...
def RegNetX_4GF(pretrained=False, **kwargs):
    model = RegNet(
        ...
        **kwargs)
    _load_pretrained(
        pretrained, model, MODEL_URLS["RegNetX_4GF"])
    return model


def RegNetX_32GF(pretrained=False, **kwargs):
    model = RegNet(
        w_a=69.86,
        w_0=320,
        w_m=2.0,
        ...
        **kwargs)
    _load_pretrained(
        pretrained, model, MODEL_URLS["RegNetX_32GF"])
    return model
...
def RegNetY_4GF(pretrained=False, **kwargs):
    model = RegNet(
        w_a=31.41,
        w_0=96,
        w_m=2.24,
